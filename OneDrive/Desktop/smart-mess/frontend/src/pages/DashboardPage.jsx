              ...
              <tr style={{ background: 'var(--primary-light)' }}>
                {['Mess', 'Feedback', 'Food', 'Taste', 'Hygiene', 'Portion', 'Overall'].map(h => (
                  <th key={h} style={{ padding: '10px 14px', textAlign: 'left',
                    color: 'var(--primary)', fontWeight: 600, whiteSpace: 'nowrap' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {stats.map((s, i) => (
                <tr key={s.mess} style={{ borderTop: '1px solid var(--border)',
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {recent.map((f, i) => (
              <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12,
                ...
                <div style={{ fontWeight: 700, color: f.overall >= 4 ? 'var(--green)' : f.overall >= 3 ? 'var(--orange)' : 'var(--red)',
